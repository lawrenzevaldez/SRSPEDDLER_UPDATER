    ...
    async saveJsonData() {
        try {
            ...
            for(let i = 0; i < pageRes.length; i++) {
                ...
                let stocks = (parseFloat(prodDetails.SellingArea) < 0) ? 0 : parseFloat(prodDetails.SellingArea)
                // let percentage = itemPrice * 0.02;
                // itemPrice = parseInt(itemPrice)+parseInt(percentage)
                // // NEW METHOD FOR UPDATE OF STOCK AND PRICE
                let res = await ProductMod.saveJson(pageRes[i].ProductID, stocks, itemPrice)
                if(res) {
                    console.log(i)
                }
                // // ./NEW METHOD FOR UPDATE OF STOCK AND PRICE
